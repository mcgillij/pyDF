...
class GameMap(object):
    ...
    def __init__(self, tw, mapw, maph, startXTile, startYTile, numXTiles, numYTiles):
        ...
        self.mapdata = [[[0 for cols in range(self.maph)] for rows in range(self.mapw)] for z in
                        range(self.zlevels)]  # [row for row in xrange(self.zlevels)]
        ...
        for x in range(int(self.startXTile), int(self.startXTile + self.numXTiles)):
            for y in range(int(self.startYTile), int(self.startYTile + self.numYTiles)):
                ...
                for z in self.mapdata[zlevel][x][y].content:
                    if z != None:
                        if z.selected == True:
                            self.tiledBG.blit(z.image,
                                              ((x - self.startXTile) * self.tw, (y - self.startYTile) * self.tw))
                            self.tiledBG.blit(self.digtileimages[3][0],
                                              ((x - self.startXTile) * self.tw, (y - self.startYTile) * self.tw))
                if val == 2:  # replace this with a big hash of images
                    image = 0
                    self.tiledBG.blit(self.digtileimages[image][0],
                                      ((x - self.startXTile) * self.tw, (y - self.startYTile) * self.tw))
                elif val == 3:
                    image = 1
                    self.tiledBG.blit(self.digtileimages[image][0],
            ...
            for y in range(int(self.startYTile), int(self.startYTile + self.numYTiles)):
                ...
                if val == 5:
                    ...
                    if self.checkMap(x, y, zlevel - 1) == 5:  # sky
                        val = 9
                self.tiledBG.blit(self.tileimages[val][0],
                                  ((x - self.startXTile) * self.tw, (y - self.startYTile) * self.tw))
                for z in self.mapdata[zlevel][x][y].content:
                    if z != None:
        ...
        for drow in (-1, 0, 1):
            for dcol in (-1, 0, 1):
                ...
                if (0 <= newrow <= self.mapw - 1 and 0 <= newcol <= self.maph - 1) and tileblocked == False:
                    ...
                    if (tilevalue == 7 and calc_z + 1 <= maxz and self.checkMap(newrow, newcol,
                                                                                calc_z + 1) == 5):  # up ramp
                        slist.append((newrow, newcol, calc_z + 1))
                elif (tilevalue == 5 and calc_z - 1 >= minz and self.checkMap(newrow, newcol,
                                                                              calc_z - 1) == 7):  # down ramp
                    slist.append((newrow, newcol, calc_z - 1))
        return slist
