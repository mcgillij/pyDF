...
class engine:
    def __init__(self):
        ...
        self.vpDimensions = (math.floor(int(0.8 * self.ww) / self.tw) * self.tw,
                             math.floor(int(0.9 * self.wh) / self.tw) * self.tw)  # resolution of the view port
        ...
        self.numXTiles = int(
            math.ceil(int(self.vpDimensions[0]) / self.tw))  # the number of tiles to be shown at one time for X
        self.numYTiles = int(
            math.ceil(int(self.vpDimensions[1]) / self.tw))  # the number of tiles to be shown at one time for y
        ...
        self.m = gamemap.GameMap(self.tw, self.mapw, self.maph, self.startXTile, self.startYTile, self.numXTiles,
                                 self.numYTiles)
        self.m.initMap()
        self.m.initEMap()
    ...
    def handle_keyboard(self, event):
        ...
        elif (event.key == K_UP or event.key == K_DOWN) and (
                keymods & pygame.KMOD_LALT and self.showmenu == True and self.currentmenu != None):
        ...
        # reset viewport
            ...
            if self.fullscreen == False:
                ...
                self.vpDimensions = (newwidth * self.tw, newheight * self.tw)  # resolution of the view port
                self.m.numXTiles = int(
                    math.ceil(int(self.vpDimensions[0]) / self.tw))  # tiles to be shown at one time for X
                self.m.numYTiles = int(
                    math.ceil(int(self.vpDimensions[1]) / self.tw))  # tiles to be shown at one time for y
                self.m.tiledBG = pygame.Surface((self.m.numXTiles * self.tw, self.m.numYTiles * self.tw)).convert()
    ...
    def handle_mouse(self):
        if 1 in self.buttons and self.editmode[0] == 'designate' and self.editmode[1] == 'itemselect':
            ...
            if mx < self.m.numXTiles * self.tw + self.vpRenderOffset[0] and my < self.m.numYTiles * self.tw + \
                    self.vpRenderOffset[1]:  # within the map viewport
                self.m.select_items((mx - self.vpRenderOffset[0] + self.vpCoordinate[0]) / self.tw,
                                    (my - self.vpRenderOffset[1] + self.vpCoordinate[1]) / self.tw, self.currentZlevel)
        ...
        if 1 in self.buttons and self.editmode[0] == 'designate' and self.editmode[1] == 'drop':
            ...
            if mx < self.m.numXTiles * self.tw + self.vpRenderOffset[0] and my < self.m.numYTiles * self.tw + \
                    self.vpRenderOffset[1]:  # within the map viewport
                self.m.updateEMap((mx - self.vpRenderOffset[0] + self.vpCoordinate[0]) / self.tw,
                                  (my - self.vpRenderOffset[1] + self.vpCoordinate[1]) / self.tw, self.currentZlevel,
                                  self.editmode)
    ...
    def handle_viewport(self):
        ...
        if self.selectmode == True:
            ...
            self.tilecontent = self.m.checkMapContent(self.selectcursor.mapx, self.selectcursor.mapy,
                                                      self.currentZlevel)
            ...
            if mob.job != None:
                if mob.position == mob.job.move:
                    if mob.job.name == "Channeling":
                        ...
                        self.m.writeMap(mob.job.dest[0], mob.job.dest[1], diglevel,
                                        digtype)  # default to a green tile for now.
                        self.m.setBlocked(mob.job.dest[0], mob.job.dest[1], diglevel, False)  # unblock the tile
                        digtype = 5  # put an empty tile on top
                        self.m.writeMap(mob.job.dest[0], mob.job.dest[1], mob.job.dest[2],
                                        digtype)  # default to a green tile for now.
                        self.m.writeEMap(mob.job.dest[0], mob.job.dest[1], mob.job.dest[2],
                                         1)  # default to a empty tile for now.
    ...
    def render(self):
        ...
        if self.selectmode == True:
            ...
            if self.testmode == True and self.roomtiles:
                for pos in self.roomtiles:
                    self.finalmap.blit(self.selectcursor.image, [pos[0] * self.tw + self.vpCoordinate[0],
            ...
            self.screen.blit(self.arialFnt.render('content: ' + str(self.tilecontent), True, self.white),
                             self.menuOffset2)
        ...
        if self.editmode != [None, None]:
            self.screen.blit(self.arialFnt.render('Digging: ' + str(self.editmode[1]), True, self.white),
                             self.digTypeOffset)

        self.screen.blit(self.finalmap, self.vpRenderOffset, (self.vpCoordinate[0] - (self.m.startXTile * self.tw), (
                    self.vpCoordinate[1] - (self.m.startYTile * self.tw))) + self.vpDimensions)
        self.screen.blit(self.arialFnt.render(
            'coordinates: ' + str(self.vpCoordinate[0]) + ", " + str(self.vpCoordinate[1]) + " Z: " + str(
                self.currentZlevel), True, self.white), self.statsOffset)
        # screen filling goes here
        for mo in self.mobs:
            if mo.position[2] == self.currentZlevel:  # only render the dorfs on the current level
                self.screen.blit(mo.image, self.vpRenderOffset, (self.vpCoordinate[0] - (mo.position[0] * self.tw), (
                            self.vpCoordinate[1] - (mo.position[1] * self.tw))) + self.vpDimensions)
