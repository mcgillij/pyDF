        ...
        for octave in range(octaves):
            ...
            for x in range(self.width):
                for y in range(self.height):
                    ...
                    for octave in range(octaves):
                        sc *= frequency
                        grey = abs(self.noise(sc * float(x) / self.width, sc * float(y) / self.height,
                                              0.0))  # Turbulence noise in theory
                    ...
                    color /= maxamplitude
                    color = int(round(
                        color * 100.0))  # If you adjust the multiplyer you need to adjust the tile mappings in gamemap.py
